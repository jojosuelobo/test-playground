...
Modal: `signup-modal` · Form: `signup-form`

- [ ] Abrir modal de cadastro pelo botão `home-signup-button` (Home) e por `nav-signup-button` (Navbar)
- [ ] Fechar o modal pelo botão `signup-modal-close-button`
- [ ] Fechar o modal clicando fora (`modal-overlay`) e pela tecla `Esc`
- [ ] Cadastro com sucesso: preencher `signup-name-input`, `signup-email-input`, `signup-password-input` (telefone `signup-phone-input` é opcional) e clicar `signup-submit-button` → redireciona para `/dashboard`, sessão criada (cookie `session`)
- [ ] Cadastro sem nome ou nome com 1 caractere → erro de validação exibido em `signup-error-message`, permanece no modal
- [ ] Cadastro com email inválido (`"nao-e-email"`) → `signup-error-message`
- [ ] Cadastro com senha curta (< 8 caracteres) → `signup-error-message`
- [ ] Cadastro com email já existente (ex: `demo@example.com`) → mensagem de email já em uso, permanece no modal
- [ ] Campo "Data de nascimento" **não existe** no formulário (removido propositalmente) — validar ausência
- [ ] Link "Cadastre-se" dentro do modal de login (`login-switch-to-signup-button`) troca para o modal de cadastro
- [ ] Link "Entrar" dentro do modal de cadastro (`signup-switch-to-login-button`) troca para o modal de login
- [ ] Usuário criado via signup sempre nasce com papel `STUDENT` (nunca vira professor pela UI)

### API — `POST /api/auth/signup`
- [ ] Payload válido → `201`, retorna `{ user: { id, name, email, role } }`, seta cookie `session`
- [ ] Payload inválido (nome curto / email inválido / senha curta) → `400`
- [ ] Email duplicado → `409`
...
Modal: `login-modal` · Form: `login-form`

- [ ] Abrir modal de login pelo `home-login-button` e `nav-login-button`
- [ ] Nota com credenciais do professor visível no modal (`login-professor-credentials-hint`)
- [ ] Login com sucesso como aluno (`demo@example.com` / `Password123!`) → redireciona para `/dashboard`
- [ ] Login com sucesso como professor (`professor@admin.com` / `admin`) → redireciona para `/professor`
- [ ] Login com senha errada → `login-error-message`, permanece no modal
- [ ] Login com email não cadastrado → `login-error-message`
- [ ] Login com campos vazios → validação do browser (`required`) impede submit

### API — `POST /api/auth/login`
- [ ] Credenciais válidas → `200`, `{ user: { id, name, email, role } }`, seta cookie
- [ ] Credenciais inválidas → `401`
- [ ] Payload malformado (sem email/senha) → `400`

---

## 3. Logout

- [ ] Clicar `nav-logout-button` estando logado → cookie `session` é removido, redireciona para `/`
- [ ] Após logout, acessar rota protegida diretamente pela URL (ex: `/dashboard`) → redireciona para `/?login=required`

### API — `POST /api/auth/logout`
- [ ] Chamada autenticada → `200`, cookie removido
- [ ] `GET /api/auth/me` sem sessão → `401`
- [ ] `GET /api/auth/me` com sessão válida → `200`, `{ user }`

---

## 4. Proteção de rotas (proxy/middleware)

- [ ] Deslogado acessando `/marketplace` → redireciona para `/?login=required`
- [ ] Deslogado acessando `/course/{id}` → redireciona
- [ ] Deslogado acessando `/dashboard` → redireciona
- [ ] Deslogado acessando `/account` → redireciona
- [ ] Deslogado acessando `/professor` → redireciona
- [ ] Deslogado acessando `/u/{userId}` (perfil público) → **permanece acessível**, sem redirecionamento
- [ ] Deslogado acessando `/` → acessível normalmente
